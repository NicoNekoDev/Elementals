...
import ro.nicuch.elementals.Elementals.Nano;
...
public class User {
	...
	private boolean elytraParkour0;
	private boolean elytraParkour1;

	public void toggleElytraParkour(boolean b) {
		this.elytraParkour0 = b;
	}

	public void toggleElytraParkourStart(boolean b) {
		this.elytraParkour1 = b;
	}

	public boolean isInElytraParkour() {
		return this.elytraParkour0;
	}

	public boolean isInElytraParkourStart() {
		return this.elytraParkour1;
	}
	...
	public User(Player base) {
		...
		this.nanoPick = Elementals.Nano.A;
		this.nanoSpade = Elementals.Nano.A;
		this.nanoAxe = Elementals.Nano.A;
		this.pvpTicks = 0;
		this.sounds = true;
		this.pack = false;
		...
		Bukkit.getScheduler().runTaskAsynchronously(Elementals.get(), () -> {
			try {
				ResultSet rs = Elementals.getBase()
						...
						.executeQuery();
				if (!rs.next()) {
					Elementals.getBase().prepareStatement("INSERT INTO pikapoints(uuid, points) VALUES('"
					...
					Bukkit.getScheduler().runTask(Elementals.get(), () -> {
						...
						ItemStack i7 = new ItemStack(Material.STONE_SPADE);
						...
						List<String> lore = Lists.newArrayList("§a§l[§6PikaCraft§a§l]");
						m1.setDisplayName("§7Casca de Piele");
						m2.setDisplayName("§7Plosa de Piele");
						m3.setDisplayName("§7Pantaloni de Piele");
						m4.setDisplayName("§7Botosi de Piele");
						m5.setDisplayName("§7Tarnacop de Piatra");
						m6.setDisplayName("§7Sabie de Piatra");
						m7.setDisplayName("§7Lopata de Piatra");
						m8.setDisplayName("§7Topor de Piatra");
						m11.setDisplayName("§bProtectie de Diamant §8(§a51x256x52§8)");
						m12.setDisplayName("§bDiamond §aof §6Pikachu");
						m1.setLore(lore);
						m2.setLore(lore);
						m3.setLore(lore);
						m4.setLore(lore);
				...
				base.sendMessage("§a§l[§6PikaCraft§a] §eEroare! Contacteaza un admin!");
			}
		});
	}
	...
	public int getServerLevel() {
		try {
			return ExperienceAPI.getPowerLevel(this.base) / 13;
	...
	public boolean hasPermission(String node) {
		if (this.base.isOp())
			return true;
		return Elementals.getPermission().has(this.base, node);
	}
	...
	public void save() {
		try {
			Elementals.getBase().prepareStatement(
