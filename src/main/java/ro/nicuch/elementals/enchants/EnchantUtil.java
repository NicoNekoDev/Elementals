...
public class EnchantUtil {
    ...
    public static enum CCurseEnchType {
        // normal
        ...
        private CCurseEnchType(String name) {
            this.name = name;
        }
    ...
    public static enum CEnchantType {
        ...
        private CEnchantType(String name, double chance) {
            this.name = name;
            this.chance = chance;
        }
        ...
        public static List<CEnchantType> getEnchantsForEnderCase() {
            List<CEnchantType> list = Lists.newArrayList(CEnchantType.BLAZE, CEnchantType.BLIND,
    ...
    public static ItemStack getBook(CEnchantType type) {
        ...
        List<String> lore = Lists.newArrayList(type.getName());
        meta.setLore(lore);
    ...
    public static ItemStack getCurseBook(CCurseEnchType type) {
        ...
        ItemMeta meta = book.getItemMeta();
        meta.setDisplayName("�eEnchanted Book");
        meta.setLore(Lists.newArrayList("�cCurse of Anti-Enchanting", "�aRemoving: " + type.getName()));
        book.setItemMeta(meta);
        return book;
    }
    ...
    public static boolean isEnchantedBook(ItemStack item, CEnchantType type) {
        ...
        if (meta.getLore().get(0) == null)
            return false;
        if (!meta.getLore().get(0).equals(type.getName()))
            return false;
        return true;
    }
    ...
    public static boolean isCurseEnchBook(ItemStack item, CCurseEnchType type) {
        ...
        if (meta.getLore().get(1) == null)
            return false;
        if (!meta.getLore().get(1).equals("�aRemoving: " + type.getName()))
            return false;
        return true;
    }
    ...
    public static ItemStack unEnchantItem(ItemStack item, CEnchantType type) {
        ...
        List<String> lore = itemMeta.getLore();
        for (int i = 0; i < lore.size(); i++)
            if (lore.get(i).equals(type.getName()))
                lore.remove(i);
        itemMeta.setLore(lore);
        if (getEnchantCount(item) == 0 && itemMeta.hasDisplayName()
                && (!itemMeta.getDisplayName().startsWith("�bNaNo")))
    ...
    public static ItemStack enchantItem(ItemStack item, CEnchantType type) {
        ...
        if (!itemMeta.hasDisplayName())
            itemMeta.setDisplayName("�b" + nms.getName());
    ...
    public static void openEnchanter(Player player, Dropper block) {
        ...
        inv.setItem(0, CaseUtil.getNullItem(DyeColor.BLACK));
        inv.setItem(1, CaseUtil.getNullItem(DyeColor.BLACK));
        inv.setItem(3, CaseUtil.getNullItem(DyeColor.BLACK, "�c�l< �bItem de Enchantat"));
        ItemStack buttonItem = CaseUtil.getNullItem(DyeColor.LIME);
        ItemMeta buttonMeta = buttonItem.getItemMeta();
        buttonMeta.setDisplayName("�aClick pentru a enchanta!");
        buttonMeta.setLore(Lists.newArrayList("�cNecesita 10 nivele de experienta!"));
        buttonItem.setItemMeta(buttonMeta);
        inv.setItem(4, buttonItem);
        inv.setItem(5, CaseUtil.getNullItem(DyeColor.BLACK, "�bCartea cu Enchant �c�l>"));
        inv.setItem(7, CaseUtil.getNullItem(DyeColor.BLACK));
        inv.setItem(8, CaseUtil.getNullItem(DyeColor.BLACK));
        ...
        switch (entity.getType()) {
            ...
            case MUSHROOM_COW:
                ...
                if (entity.getFireTicks() > 0)
                    ...
                    drops.add(new ItemStack(Material.RAW_BEEF, ElementalsUtil.nextInt(level)));
                break;
            case HORSE:
                drops.add(new ItemStack(Material.LEATHER, ElementalsUtil.nextInt(level)));
                break;
            ...
            case PIG:
                if (entity.getFireTicks() > 0)
                    drops.add(new ItemStack(Material.GRILLED_PORK, ElementalsUtil.nextInt(level)));
                else
                    drops.add(new ItemStack(Material.PORK, ElementalsUtil.nextInt(level)));
                break;
            case POLAR_BEAR:
                if (entity.getFireTicks() > 0)
                    drops.add(new ItemStack(Material.COOKED_FISH, ElementalsUtil.nextInt(level)));
                else
                    drops.add(new ItemStack(Material.RAW_FISH, ElementalsUtil.nextInt(level)));
                break;
            case RABBIT:
            ...
            case SHEEP:
                ...
                break;
            case SQUID:
                drops.add(new ItemStack(Material.INK_SACK, ElementalsUtil.nextInt(level)));
                break;
            case BLAZE:
            ...
            case CREEPER:
            case GHAST:
                drops.add(new ItemStack(Material.SULPHUR, ElementalsUtil.nextInt(level)));
                break;
            case ENDERMAN:
    ...
    public static void useEnchant(Event event, Entity ent_v, Entity ent_d, DamageCause cause) {
        ...
        if (ent_d instanceof LivingEntity) {
            if (ent_v.getWorld().getName().equals("spawn"))
                if (!WGConfigLoc.checkPvP(ent_v.getLocation()))
                    return;
            if (ent_d.getWorld().getName().equals("spawn"))
                if (!WGConfigLoc.checkPvP(ent_d.getLocation()))
                    return;
            if (ent_d.isDead())
                return;
            ...
            if (event instanceof EntityDamageByEntityEvent) {
                ...
                if (arrow.hasMetadata("pesticide3") && isArthropod(ent_v))
                    hitEvent.setDamage(hitEvent.getDamage() + 3.75);
            } else if (event instanceof FakeEntityDamageByEntityEvent) {
                FakeEntityDamageByEntityEvent hitEvent = (FakeEntityDamageByEntityEvent) event;
                if (arrow.hasMetadata("necromancer1") && isUndead(ent_v))
                    hitEvent.setDamage(hitEvent.getDamage() + 1.25);
                else if (arrow.hasMetadata("necromancer2") && isUndead(ent_v))
                    hitEvent.setDamage(hitEvent.getDamage() + 2.5);
    ...
    public static boolean hasEnchant(ItemStack item, Enchantment type, int level) {
        if (!item.containsEnchantment(type))
            return false;
        if (item.getEnchantmentLevel(type) != level)
            return false;
        return true;
    }
    ...
    public static boolean hasEnchant(ItemStack item, CEnchantType type) {
        ...
        if (item.getItemMeta().getLore().isEmpty())
            return false;
        if (!item.getItemMeta().getLore().contains(type.getName()))
            return false;
        return true;
    }
